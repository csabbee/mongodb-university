...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */
        ...
        var aggregation = this.db.collection('item').aggregate([
            { $group: { _id: "$category", num: { $sum: 1 } } },
            { $sort: { _id: 1 } }
        ]);

        aggregation.toArray((err, result) => {

            allCategory.num = result.reduce((previousCount, currentAggregationItem) => {
                return previousCount + currentAggregationItem.num;
            }, 0);

            callback(_.concat(categories, allCategory, result));
        });
    };
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
        ...
        var cursor = this.db.collection('item').find(getCategory(category));
